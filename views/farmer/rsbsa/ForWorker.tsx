...
const ForWorker = () => {
	...
	useEffect(() => {
		userProfile.laborer_activity.length
			? setIsSelected(userProfile.laborer_activity)
			: setIsSelected("");
	}, []);
		...
		<div className="flex flex-col gap-y-5 font-noto text-gray-700">
			...
			<div className="grid grid-cols-2 gap-16">
				...
				<LivelihoodOption
					...
					onClickOption={() =>
						handleOptionClick({ name: "workerActivity", value: "Cultivation" })
					}
				/>
				<LivelihoodOption
					selectedOption={isSelected}
					...
					onClickOption={() =>
						handleOptionClick({ name: "workerActivity", value: "Harvesting" })
					}
				/>
			</div>
			{hasError && (
